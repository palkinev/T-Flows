      ...
      do sect = 1, cgns_base(base) % block(block) % n_sects
        ...
        if ( ElementTypeName(cell_type) .eq. 'HEXA_8' ) then
          ...
          do c = cgns_base(base) % block(block) % section(sect) % first_cell,  &
            ...
            call Swap_Int(grid % cells_bnd_color(1,c),  &
                          grid % cells_bnd_color(5,c))

            call Swap_Int(grid % cells_bnd_color(2,c),  &
                          grid % cells_bnd_color(1,c))

            call Swap_Int(grid % cells_bnd_color(3,c),  &
                          grid % cells_bnd_color(2,c))

            call Swap_Int(grid % cells_bnd_color(4,c),  &
                          grid % cells_bnd_color(3,c))

            call Swap_Int(grid % cells_bnd_color(5,c),  &
                          grid % cells_bnd_color(4,c))
