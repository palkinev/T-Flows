...
!==============================================================================!
  ...
  do s = 1, grid % n_faces
    ...
    if(u % bnd_cond_type(c2) .eq. OUTFLOW) then
      u % n(c2) = u % n(c1)
      v % n(c2) = v % n(c1)
      w % n(c2) = w % n(c1)
      m_flux % n(s) = flow % density_f(s) * ( u % n(c2)*grid % sx(s)    &
    ...
    if(u % bnd_cond_type(c2) .eq. CONVECT  &
       .and. m_flux % n(s) > 0.0) then
      m_flux % n(s) = flow % density_f(s) * ( u % n(c2)*grid % sx(s)    &
                                            + v % n(c2)*grid % sy(s)    &
                                            + w % n(c2)*grid % sz(s) )
    ...
    if(u % bnd_cond_type(c2) .eq. PRESSURE  &
       .and. m_flux % n(s) >0.0) then
      bulk % mass_out = bulk % mass_out + m_flux % n(s)
    end if
  end do  ! 1, grid % n_faces
  ...
  do s = 1, grid % n_faces
    ...
    if(u % bnd_cond_type(c2) .eq. OUTFLOW  .or.  &
       u % bnd_cond_type(c2) .eq. CONVECT  .or.  &
       u % bnd_cond_type(c2) .eq. PRESSURE) then
      u % n(c2) = u % n(c2) * fac
      v % n(c2) = v % n(c2) * fac
      w % n(c2) = w % n(c2) * fac
